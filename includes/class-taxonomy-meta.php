...
class K2_Taxonomy_Meta
{
    ...
    function __construct($redux)
    {
        ...
        do_action('k2_taxonomy_meta_register', $this);
    ...
    function admin_init()
    {
        ...
        if ('term.php' == $pagenow) {
            $this->notices = get_transient('k2-taxonomy-meta-transients');

            // If the transient exists then we override it with an empty value
            if (false !== $this->notices) {
                set_transient('k2-taxonomy-meta-transients', '');
            } else {
    ...
    {
        $this->default_args['opt_name'] = isset($this->default_args['opt_name']) ? $this->default_args['opt_name'] . '_taxonomy_metabox' : 'k2_taxonomy_metabox';
        $this->default_args['display_name'] = esc_html__('Settings', K2_TEXT_DOMAIN);
    ...
    function add_section($taxonomy, $section = array())
    {
        ...
        foreach ($section['fields'] as $fkey => $field) {
            ...
            if (in_array($field['id'], $this->field_ids[$taxonomy])) {
                trigger_error(sprintf(esc_html__('The field with id %1$s for taxonomy %2$s is already registered.', K2_TEXT_DOMAIN), esc_html($field['id'], K2_TEXT_DOMAIN), esc_html($taxonomy, K2_TEXT_DOMAIN)));
        ...
        if (!empty($section['id'])) {
            if ($this->section_exist($section['id'], $taxonomy)) {
                trigger_error(sprintf(esc_html__('Section %1$s for taxonomy %2$s is already exist.', K2_TEXT_DOMAIN), esc_html($field['id']), esc_html($taxonomy)));
                return;
            } else {
    ...
    {
        wp_nonce_field('k2_taxonomy_metabox_nonce_action', 'k2_taxonomy_metabox_nonce');
...
//        echo '</div>';
    ...
    function panel_scripts()
    {
        ...
        wp_enqueue_style('k2-metabox', k2core()->path('APP_URL') . '/assets/css/metabox' . Redux_Functions::isMin() . '.css', array(), self::$version, 'all');
        wp_enqueue_script('k2-metabox', k2core()->path('APP_URL') . '/assets/js/metabox' . Redux_Functions::isMin() . '.js', array('jquery', 'redux-js'), self::$version, 'all');
    ...
    {
        return k2core()->path('APP_DIR') . '/templates/panel-post-meta';
    }
    ...
    protected function save_term_meta($term_id, $tt_id, $taxonomy, $transient = false)
    {
        ...
        if (empty($_POST[$opt_name]) || empty($_POST['k2_taxonomy_metabox_nonce']) || !wp_verify_nonce($_POST['k2_taxonomy_metabox_nonce'], 'k2_taxonomy_metabox_nonce_action')) {
            return;
        }
        ...
        if ($transient) {
            ...
            if (!empty($redux->errors) || !empty($redux->warnings)) {
                ...
                set_transient('k2-taxonomy-metabox-transients', $notices);
            }
        }
    }
...
//            echo '<p>';
//            esc_html_e('All invalid custom field values for newly created term or existing term will not be saved, if you are missing something, please check field descriptions for instructions how to input valid values.', K2_TEXT_DOMAIN);
//            echo '</p>';
...
//        }
        ...
        if (!empty($this->notices['errors'])) {
            ...
            printf(
                ...
                esc_html__('error(s)', K2_TEXT_DOMAIN),
                esc_html__('were found! Some data might not be saved.', K2_TEXT_DOMAIN)
            );
            echo '</p></div>';
        }
        ...
        if (!empty($this->notices['warnings'])) {
            ...
            printf(
                ...
                esc_html__('warning(s)', K2_TEXT_DOMAIN),
                esc_html__('were found! Some data might not be saved.', K2_TEXT_DOMAIN)
            );
    ...
    protected function generate_output_css($taxonomy, $options)
    {
        ...
        echo '<style type="text/css" id="k2-' . $taxonomy . '-dynamic-css">' . $redux->outputCSS . '</style>';
    }
}
