...
class K2_Post_Metabox
{
    ...
    function __construct($redux)
    {
        ...
        do_action('k2_post_metabox_register', $this);
        ...
        if (!empty($this->panels)) {
            ...
            add_action('hidden_meta_boxes', array($this, 'k2_set_user_metaboxes'));
        }
    }
    ...
    function admin_init()
    {
        ...
        if ('post.php' == $pagenow) {
            $this->notices = get_transient('k2-post-metabox-transients');

            // If the transient exists then we override it with an empty value
            if (false !== $this->notices) {
                set_transient('k2-post-metabox-transients', '');
            } else {
...
//        $this->default_args['opt_name'] = isset($this->default_args['opt_name']) ? $this->default_args['opt_name'] . '_post_metabox' : 'abtheme_post_metabox';
    ...
    function add_section($post_type, $section = array())
    {
        ...
        foreach ($section['fields'] as $fkey => $field) {
            ...
            if (in_array($field['id'], $this->field_ids[$post_type])) {
                trigger_error(esc_html__('The field with id',K2_TEXT_DOMAIN).' '.esc_html($field['id']).' ' .esc_html__('for post type',K2_TEXT_DOMAIN).' '.esc_html($post_type).' '.esc_html__('is already registered.', K2_TEXT_DOMAIN));
        ...
        if (!empty($section['id'])) {
            if ($this->section_exist($section['id'], $post_type)) {
                trigger_error(esc_html__('Section',K2_TEXT_DOMAIN).' '.esc_html($field['id']).' ' .esc_html__('for post type',K2_TEXT_DOMAIN).' '.esc_html($post_type).' '.esc_html__('is already registered.', K2_TEXT_DOMAIN));
                return;
            } else {
    ...
    function add_meta_boxes()
    {
        ...
        foreach ($this->panels as $post_type => $panel) {
            ...
            if (in_array(str_replace('k2_pf_', '', $new_post_type), $post_formats) !== false) {
                $new_post_type = 'post';
            }
            add_meta_box(
                $panel['args']['opt_name'],
    ...
    function k2_set_user_metaboxes($hidden)
    {
...
//        die();
    ...
    function save_meta_boxes($post_id, $post)
    {
        ...
        if(!empty($post_format)){
            $k2_pf_panel = 'k2_pf_'.$post_format;
            $post_format_type = !empty($_POST['post_format_' . $post_format]) ? $_POST['post_format_' . $post_format] : '';
            if (in_array($k2_pf_panel, $this->post_types) && !empty($_POST[$this->panels[$k2_pf_panel]['args']['opt_name']]) && !empty($post_format_type)) {
                $sections_post_format = $this->get_opt_sections($k2_pf_panel);
                $args_post_format = $this->get_opt_args($k2_pf_panel);
                $data_to_save_pfm = array();
                $data_to_compare_pfm = $this->get_opt_defaults($k2_pf_panel);
        ...
        if (!empty($redux->errors) || !empty($redux->warnings)) {
            ...
            set_transient('k2-post-metabox-transients', $notices);
        }
    }
    ...
    protected function generate_output_css($post, $options)
    {
        ...
        printf(
            ...
            esc_attr('k2-' . $post->post_type . '-dynamic-css'),
            $redux->outputCSS
        );
    }
}